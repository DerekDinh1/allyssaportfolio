...
const SECTIONS_DIR = path.join(ROOT, 'mockups', 'sections');
const OUT_DIR = path.join(ROOT, 'mockups', 'previews');
...
async function main() {
  ...
  const sectionFiles = (await readdir(SECTIONS_DIR)).filter((f) => f.endsWith('.html'));
  const available = new Set(sectionFiles.map((f) => f.replace(/\.html$/, '')));
  const missingSections = EXPECTED.filter((s) => !available.has(s));
  if (missingSections.length) warn(`expected sections not found: ${missingSections.join(', ')}`);
  const pages = EXPECTED.filter((slug) => available.has(slug)).map((slug) => ({ slug, file: path.join(SECTIONS_DIR, `${slug}.html`) }));
  for (const extra of EXTRA_PAGES) {
    if (existsSync(extra.file)) pages.push(extra);
    else warn(`optional page not found: ${path.relative(ROOT, extra.file)}`);
  }
  ...
  try {
    ...
    for (const page of pages) {
      ...
      report.pages[slug] = { source: path.relative(ROOT, file), views: {} };
      for (const vpName of Object.keys(VIEWPORTS)) {
        log(`render ${slug} @ ${vpName}`);
        await renderOne(ctx.base, file, slug, vpName, report);
      }
    }
  } catch (e) {
    setupFailed = true;
